...
$(document).ready(function() {
	...
	function init() {
		...
		bestem_opgave = Math.round(Math.random() * 1);
	...
	function tjek_svar() {
		if (opgavetype != 0) {
			if (parseFloat(decimalresult) == parseFloat($(".dec_input").val())) {
				dec_correct = true;
				correct($(".dec_input"));
			} else {
				wrong($(".dec_input"));
...
function updatebar() {
	...
	$(".bar").css("width", overallscore * 4.5 + 10 + "px");
...
function correct(inputfelt) {
	...
	if (dec_correct == true && pro_correct == true) {
		dec_correct = false;
		pro_correct = false;
		brok_correct = false;
